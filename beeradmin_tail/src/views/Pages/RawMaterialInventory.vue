...
async function fetchInventory() {
  try {
    ...
    const { data, error } = await supabase
      .from('inv_inventory')
      .select('id, qty, uom_id, warehouse_id, material:material_id(id, code, name, category, uom_id), uom:uom_id(code)')
    if (error) throw error
    rows.value = (data ?? [])
      .filter((row: any) => ALLOWED_CATEGORIES.includes(row.material?.category))
      .map((row: any) => {
        const warehouseOption = warehouseMap.value.get(row.warehouse_id ?? '')
        return {
          id: row.id,
