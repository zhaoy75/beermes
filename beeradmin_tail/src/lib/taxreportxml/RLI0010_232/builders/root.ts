...
import { buildLia230Xml } from './lia230'
import { buildLia240Xml } from './lia240'
import { buildLia250Xml } from './lia250'
...
export function buildXml(input: RLI0010_232_Input) {
  ...
  const lia230Pages = buildLia230Xml(input)
  const lia240Pages = buildLia240Xml(input)
  const lia250Pages = buildLia250Xml(input)
  ...
  const formIds = [
    ...
    ...lia230Pages.map((_, index) => buildLia230Id(index + 1)),
    ...lia240Pages.map((_, index) => buildLia240Id(index + 1)),
    ...lia250Pages.map((_, index) => buildLia250Id(index + 1)),
